...
def test_flask_version():
    ...
    This test verifies that the installed Flask version is 2.3.2, which is
    the version required by the application as specified in requirements.txt.
    """
    assert flask.__version__ == '2.3.2', f"Expected Flask version 2.3.2, but got {flask.__version__}"

def test_werkzeug_version():
    """
    Test that the installed Werkzeug version is compatible with the application.
    
    This test verifies that the installed Werkzeug version is 2.3.8, which is
    the version required by the application as specified in requirements.txt.
    """
    # Werkzeug 2.3.8 has a __version__ attribute
    assert hasattr(werkzeug, '__version__'), "Werkzeug does not have a __version__ attribute"
    assert werkzeug.__version__ == '2.3.8', f"Expected Werkzeug version 2.3.8, but got {werkzeug.__version__}"
